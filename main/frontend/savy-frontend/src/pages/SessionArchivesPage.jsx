...
export default function SessionArchivesPage() {
  ...
  <main className="flex-1 ml-[260px] p-8 space-y-8 overflow-y-auto bg-slate-950 min-h-screen">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Session Archives</h1>
          <p className="text-slate-400 mt-2">View and manage all recorded EEG sessions</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">{totalSize.toFixed(1)} GB</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-400">Consented</p>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
        <table className="w-full">
          <thead className="bg-slate-950 border-b border-slate-800">
            <tr>
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase">Session ID</th>
              ...
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase">Duration</th>
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase">Size</th>
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase">Status</th>
              <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase">Consent</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {filteredSessions.map((session) => (
              <tr key={session.id} className="hover:bg-slate-800/50 transition-colors">
                <td className="px-6 py-4 text-sm font-medium text-cyan-400">{session.id}</td>
                <td className="px-6 py-4">
                  <div>
                    <p className="text-sm font-medium text-white">{session.patientName}</p>
                    <p className="text-xs text-slate-500">{session.patientId}</p>
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-slate-400">{session.date}</td>
                <td className="px-6 py-4 text-sm text-slate-400">{session.type}</td>
                <td className="px-6 py-4 text-sm text-slate-400">{session.duration}</td>
                <td className="px-6 py-4 text-sm text-slate-400">{session.dataSize}</td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <span className={`text-sm font-medium ${
                    session.consent === 'Granted' ? 'text-green-400' :
                    session.consent === 'Pending' ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>
                    {session.consent}
                  </span>
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-3">
                    <button className="text-slate-400 hover:text-cyan-400 transition-colors" title="View">
                      <EyeIcon />
                    </button>
                    <button className="text-slate-400 hover:text-cyan-400 transition-colors" title="Download">
                      <DownloadIcon />
                    </button>
                    <button className="text-slate-400 hover:text-cyan-400 transition-colors" title="Blockchain Record">
                      <ShieldIcon />
                    </button>
                  </div>
                </td>
